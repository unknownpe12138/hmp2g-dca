...
function choose_geometry(type){
    if (!window.glb.base_geometry[type]){
        // Auto-create basic geometries for DCA visualization
        if (type=='box'){
            window.glb.base_geometry[type] = new THREE.BoxGeometry(1, 1, 1);
            console.log('Auto-created box geometry');
        }else if(type=='sphe' || type=='ball' || type=='sphere'){
            window.glb.base_geometry[type] = new THREE.SphereGeometry(1);
            console.log('Auto-created sphere geometry');
        }else if(type=='cone'){
            window.glb.base_geometry[type] = new THREE.ConeGeometry(1, 2*1);
            console.log('Auto-created cone geometry');
        }else if(type=='tower' || type=='tower2'){
            // Cylinder for towers
            window.glb.base_geometry[type] = new THREE.CylinderGeometry(0.5, 0.5, 1, 32);
            console.log('Auto-created tower geometry');
        }else{
